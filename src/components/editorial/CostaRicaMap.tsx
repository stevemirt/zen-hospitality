...
// Zen Reserve locations — coords + animation delay come from the component,
// names + subtitles come from i18n so they localize per locale.
const REGION_COORDS = [
  { x: 77, y: 145, delay: 0 },
  { x: 70, y: 203, delay: 0.15 },
  { x: 77, y: 251, delay: 0.3 },
];

export function CostaRicaMap() {
  const t = useTranslations("whyCostaRica");
  const markerI18n = t.raw("mapMarkers") as Array<{ name: string; subtitle: string }>;
  const REGIONS = REGION_COORDS.map((c, i) => ({
    ...c,
    name: markerI18n[i]?.name ?? "",
    subtitle: markerI18n[i]?.subtitle ?? "",
  }));
